...
\[
\rho\frac{D^n}{K{_D}^n + D^n}(1-S)-\mu{_S}S=temps^{-1} (h^{-1}) \times \frac{concentration^n (mg/kg)^n}{concentration^n (mg/kg)^n 
+ concentration^n (mg/kg)^n}(1-intensité (sans unité))-temps (h^{-1})\times intensité (sans unité).
\]
...
\begin{itemize}
    ...
    positive jusqu'à atteindre 0, valeur à laquelle elle restera, d'après la formule, constante.}
