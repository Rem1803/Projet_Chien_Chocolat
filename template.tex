...
\midrule
Physiology lead & Biological mechanism and physiological consistency & Marion \\
Mathematics/modeling lead & Variables, equations, assumptions, analysis & Van \\
Computation/simulation lead & Code, numerical simulations, figures & Capucine \\
Critical assumptions lead & Assumptions, limitations, uncertainty & Olinda \\
Communication/report lead & Writing, structure, clarity & Doriane \\
Coordinator & Organization, integration, deadlines & Coppélia \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{guidancebox}
This section should come mainly from Jalon 1. It explains what system you are
modeling and why the question matters.
\end{guidancebox}

\subsection{Biological context}

Describe the biological, physiological, neural, or ecotoxicological system.

Questions to answer:

\begin{itemize}
    \item What is the system?
    \item Why is it biologically relevant?
    \item What process, response, or transition are you trying to understand?
    \item What is the relevant time scale?
\end{itemize}
...
\begin{quote}
\itshape
How does a decrease in clearance affect the accumulation and recovery time of
...
\begin{quote}
\itshape
Can positive feedback create two stable regimes corresponding to low and high
activation?
\end{quote}

\subsection{Reference regime and perturbation}

Define the reference regime.

Examples:

\begin{itemize}
    \item no exposure;
    \item basal hormone level;
    \item healthy equilibrium;
    \item resting physiological condition;
    \item untreated condition;
    \item low-inflammatory state.
\end{itemize}

Then define the perturbation studied.

Examples:

\begin{itemize}
    \item increased exposure;
    \item reduced clearance;
    \item receptor blockade;
    \item stronger feedback;
    \item delayed treatment;
    \item repeated stimulation.
\end{itemize}
...
\begin{itemize}
    \item Inside the model:
    \begin{itemize}
        \item {[Example: blood compartment]}
        \item {[Example: target tissue]}
        \item {[Example: inflammatory response]}
    \end{itemize}

    \item Outside the model:
    \begin{itemize}
        \item {[Example: external exposure]}
        \item {[Example: environmental forcing]}
        \item {[Example: individual genetic variability]}
    \end{itemize}
\end{itemize}
...
\midrule
\(X(t)\) & {[Meaning of first state variable]} & {[unit]} \\
\(Y(t)\) & {[Meaning of second state variable, if any]} & {[unit]} \\
\(Z(t)\) & {[Meaning of third state variable, if any]} & {[unit]} \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{Symbol} & \textbf{Meaning} & \textbf{Unit} \\
\midrule
\(X(t)\) & \emph{Meaning of first state variable} & \emph{unit} \\
\(Y(t)\) & \emph{Meaning of second state variable, if any} & \emph{unit} \\
\(Z(t)\) & \emph{Meaning of third state variable, if any} & \emph{unit} \\
\bottomrule
\end{tabularx}
\end{center}
